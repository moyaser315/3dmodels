...
def analyze_mesh_dataset(base_path, batch_size=20):
    ...
    for i in range(0, len(remaining_files), batch_size):
        ...
        for patient, file_path in tqdm(batch):
            try:
                ...
                is_manifold = (
                    ...
                    and not has_non_manifold_edges
                )
                ...
                running_stats["inconsistent_winding"] += (
                    0 if mesh.is_winding_consistent else 1
                )
                running_stats["non_manifold_files"] += 0 if is_manifold else 1
                running_stats["files_with_non_manifold_edges"] += (
                    1 if has_non_manifold_edges else 0
                )
        ...
        current_summary = {
            ...
            "std_vertices": (
                np.sqrt(
                    (running_stats["vertex_sq_sum"] / n)
                    - (running_stats["vertex_sum"] / n) ** 2
                )
                if n > 0
                else 0
            ),
            ...
            "std_faces": (
                np.sqrt(
                    (running_stats["face_sq_sum"] / n)
                    - (running_stats["face_sum"] / n) ** 2
                )
                if n > 0
                else 0
            ),
            ...
            "files_with_non_manifold_edges": running_stats[
                "files_with_non_manifold_edges"
            ],
    ...
    pd.DataFrame([current_summary]).to_csv(
        "mesh_analysis_summary_part1.csv", index=False
    )
